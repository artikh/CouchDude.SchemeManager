...
task build -depends clean, installPackages {
    exec { msbuild "$rootDir\SchemeManager.sln" /nologo /p:Config=$config /p:Platform='Any Cpu' /maxcpucount /verbosity:minimal }    
}


task test -depends build {
    $outputFileName = "$buildDir\test-results-$version.html"
    exec { ..\Tools\xunit\xunit.console.clr4.x86.exe "$rootDir\Tests\bin\$config\CouchDude.SchemeManager.Tests.dll" /silent /-trait "level=integration" /html $outputFileName }
...
task package -depends build {
    prepareAndPackage -templateNuSpec "$srcDir\Core\CouchDude.SchemeManager.nuspec" -fileTemplates ("$srcDir\Core\bin\$config\CouchDude.*")
    TeamCity-PublishArtifact "$buildDir\CouchDude.SchemeManager.$version.nupkg"
}